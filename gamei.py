...
block_images = {"TL": load_image("assets/tiles/top_left.png"),
                ...
                "UW": load_image("assets/tiles/iceWaterDeep.png")}

bird_img = load_image("assets/Angry_Bird_red.png")

coin_img = load_image("assets/items/coin.png")
anticoin_img = load_image("assets/anticoin.png")
cookie_img = load_image("assets/cookie.png")
hearrt_img = load_image("assets/heart.png")
heart_empty_img = load_image("assets/empty_heart.png")
heart_img = load_image("assets/items/bandaid.png")
oneup_img = load_image("assets/items/first_aid.png")
poison_img = load_image("assets/Poison_Mushroom_SMW3D.png")
flash_img = load_image("assets/flash.png")
...
seabear_img = load_image("assets/sea_bear.png")
seabear_images = [seabear_img]
...
class Character(Entity):
    ...
    def jump(self, blocks, level):
        
             
        self.rect.y += 1

        if level.is_underwater == 0:
    ...
    def move_and_process_birds(self, birds):
        hit_list = pygame.sprite.spritecollide(self, birds, False)

        if len(hit_list) > 0 and self.invincibility == 0:
            if self.vy > 0:
                [b.kill() for b in hit_list]
...
class Enemy(Entity):
    ...
    def move_and_process_birds(self):
        pass
...
class Flash(Entity):
     ...
     def apply(self, character):
         ...
         character.speed = min(character.speed, character.max_speed)
         character.jump_power = min(character.jump_power, character.max_jump_power)
...
class Level():
    ...
    def __init__(self, file_path):
        ...
        for item in map_data['birds']:
            x, y = item[0] * GRID_SIZE, item[1] * GRID_SIZE
            self.starting_enemies.append(Bird(x, y, bird_img))

        for item in map_data['bears']:
            x, y = item[0] * GRID_SIZE, item[1] * GRID_SIZE
            self.starting_enemies.append(Bear(x, y, bear_images))

        for item in map_data['seabears']:
            x, y = item[0] * GRID_SIZE, item[1] * GRID_SIZE
            self.starting_enemies.append(Seabear(x, y, seabear_images))
        ...
        for item in map_data['cookies']:
            x, y = item[0] * GRID_SIZE, item[1] * GRID_SIZE
            self.starting_coins.append(Cookie(x, y, cookie_img))

        for item in map_data['anticoins']:
            x, y = item[0] * GRID_SIZE, item[1] * GRID_SIZE
            self.starting_coins.append(Anticoin(x, y, anticoin_img))
        ...
        for item in map_data['flashes']:
            x, y = item[0] * GRID_SIZE, item[1] * GRID_SIZE
            self.starting_powerups.append(Flash(x, y, flash_img))
        for item in map_data['poisons']:
            x, y = item[0] * GRID_SIZE, item[1] * GRID_SIZE
            self.starting_powerups.append(Poison
                                          (x, y, poison_img))
        ...
        self.is_underwater = map_data['is_underwater']
...
class Game():
    ...
    def display_stats(self, surface):
        ...
        lives_img = load_image("assets/character/adventurer_walk1.png", 32, 32)
        ...
        if self.hero.lives <= 9:
            hudnum = load_image("assets/character/adventurer_walk1.png".format(self.hero.lives))
            surface.blit(hudnum, (32 , 64))
        else:
        ...
        surface.blit(lives_text, (WIDTH - lives_text.get_width() - 1028, 70))
        ...
        for i in multi[:self.hero.hearts]:
            surface.blit(hearrt_img, (spacing * i, 0))
